  ...
  useEffect(() => {
    const validateSession = async () => {
      try {
        ...
        const validateUrl = buildApiUrl(
          `/auth/sessions/validate?user_id=${userDbId}&session_token=${sessionToken}`,
        );
    ...
    // 자신이 강제 로그아웃 대상인지 확인
    const isForceLogoutTarget =
      terminatedUserId && currentUserId === parseInt(terminatedUserId);

    let interval;

    if (isForceLogoutTarget) {
      console.warn(
        "⚠️ 현재 사용자가 강제 로그아웃 대상입니다! 즉시 검증 + 5초 주기로 설정",
      );
